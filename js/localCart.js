...
document.getElementById("checkout-btn").addEventListener("click", function () { window.location = "../html/checkout.html" });
var cartTotal = 0.00;
    ...
    for (var i = 0; i < cartData.length; i++) {
        ...
        for (var i = 0; i < cartItems.length; i++) {
            if (cartItems[i].innerHTML == "" || cartItems[i].innerHTML == "Your Cart is Empty") {
                cartItems[i].innerHTML = itemName + "<br> $" + itemPrice;
...
function addToCart(currItem) {
    ...
    for (var i = 0; i < cartItems.length; i++) {
        if (cartItems[i].innerHTML == "" || cartItems[i].innerHTML == "Your Cart is Empty") {
            cartItems[i].innerHTML = itemName[0].innerHTML + "<br>" + itemPrice[0].innerHTML;
            cartTotal += parseFloat(itemPrice[0].innerHTML.replace("$", "")).toFixed(2) * 1;
            break;
        }
        else if (i == (cartItems.length - 1)) {
            alert("Sorry, Your cart is full");
...
function removeCartItem(currItem) {
    ...
    if (cartData != null) {
        for (var i = 0; i < cartData.length; i++) {
            var itemInfo = cartData[i].split(",");
            if (currItemName.includes(itemInfo[0])) {
                cartData.splice(i, 1);
                cartTotal -= parseFloat(itemInfo[1].replace("$", "")).toFixed(2) * 1;
                updateLocalCart();
                break;
            }
        }
